...
- Windows qcontrol upstream builds are not published yet, so Windows wrapper/MSI builds require a manually supplied `bin\qcontrol.bin`.
...
The Windows MSI does not download `qcontrol.exe`, install it as a separate file, register a Windows Service, or run `qctl install-system`. Windows upstream qcontrol builds are not published yet, so `bin\qcontrol.bin` remains a manual local prerequisite and is embedded into the compiled wrapper by Bun.
